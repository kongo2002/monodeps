...
fn graph(opts: Opts) {
    if let Err(err) = calculate_graph(opts) {
        eprintln!("failed to calculate service dependency graph: {err}");
        std::process::exit(1);
    }
}

fn calculate_graph(opts: Opts) -> Result<()> {
...
fn unique_service_names(services: Vec<Service>) -> HashMap<ServiceIdent, Service> {
    // TODO: actually try checking uniqueness
    services
        .into_iter()
        .enumerate()
        .map(|(idx, svc)| {
            let pb = PathBuf::from(&svc.path.canonicalized);
            let name = pb
                .file_name()
                .and_then(|n| n.to_str())
                .map(|n| n.to_string())
